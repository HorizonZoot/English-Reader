...
- **句子操作**：浮窗直接提供朗读、句子翻译、解释与全文翻译入口，也可就地显示或隐藏已有译文。
- **双语对照**：英文主文本 + 克制的中文次文本，根据翻译时的分段方式按原段落或对照块显示。
...
- **整书完整性校验**：章节资源读取、解码或解析失败时中止导入，不静默跳过损坏章节。

### 文章整理与编辑

- **编辑独立文章**：可从文章列表或阅读页修改标题与正文；暂不支持编辑书籍章节。
- **段落整理预览**：支持「按换行分段」，先检查预览，再应用到编辑草稿，最后保存。仅适合每行本来就是一个段落的文章，不适用于普通排版折行；预览本身不会保存文章或发起翻译。
- **保存影响**：仅修改标题会保留译文；修改正文需要确认，会清除旧译文、终止未完成的全文翻译任务，并将阅读位置重置到开头。文章和已收藏的生词保留。

### AI（需自备 API Key，默认关闭）

- **多 Profile 管理**：可配置多个服务商，随时切换活跃 Profile。内置 DeepSeek、Kimi、智谱的默认值，也支持任意 **OpenAI 兼容**端点。
- **句子解释 / 句子翻译 / 整篇解释**：在阅读页浮窗内直接触发。
- **全文翻译**：按对照块翻译整篇文章或整本书的全部章节，**像可恢复下载一样**持久化进度——中断后可继续未完成的部分，保留已成功的对照块；失败项可单独重试；全部成功后才一次性写入文章译文。
- **分段方式**：支持「自动对照分块」「按换行」「保留原段落」。选择按文章或章节记忆，不改写原文，也不会立即重新翻译；整本书分别使用各章保存的方式。
- **翻译前确认**：预览原段数、对照块数和最多请求次数。重新生成需要确认，可能新增付费请求；旧译文保留到新任务全部成功后再整体替换。
- **任务冲突处理**：同一篇文章已有未完成的单篇或整书翻译任务时，引导查看或手动取消已有任务，不自动取消，也不新建重叠任务。
- **配置引导与错误提示**：缺少 AI 配置时可直接前往设置；服务商返回输出长度截断标记时，提示内容不完整，不将其作为完整译文或解释保存。
...
| DI | Hilt 2.50 |
| 持久化 | Room 2.6.1（**schema v7**，导出 schema + 迁移测试）、DataStore Preferences 1.0.0 |
...
│   │   └── 3.json · 4.json · 5.json · 6.json · 7.json  # 导出的 Room schema（v1/v2 已不可考）
│   ├── src/
│   │   ├── main/
...
│   │   │   │   │   ├── screen/                 # 阅读、书架、目录、文章编辑、生词本、设置
│   │   │   │   │   └── theme/                  # Material 3 主题
│   │   │   │   ├── util/                       # 分句、分段、对照分块规划、词形还原、TTS、缓存键
