...
| 限制 | 说明 |
| --- | --- |
| **极少数整本 EPUB 仍导不进来** | 用 32 本真实语料跑生产解析器，31 本可导入（97%）。超长章节会按段落边界切成多篇，超长段落按句子边界切，所以「章节比上限长」不再拒整本书。唯一失败的是《尤利西斯》：Molly Bloom 那段 21,381 字符的独白没有任何句子边界可切，而切到句子以下会把正文改坏。详见 ADR-013。 |
| **在线词典在中国大陆不可用** | Free Dictionary API 对大陆网络不可达。离线词典不受影响。 |
| **全文翻译的进程恢复有延迟** | 依赖 5 分钟 lease 过期，不做启动时主动扫描；进程被杀后需用户下次打开该文章才继续。 |
| **安装架构** | 当前 APK 仅支持 arm64-v8a。内置模型首次使用时解包到应用内部存储；额外模型的下载与解包还需要临时空间。 |
